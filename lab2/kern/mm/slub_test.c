...
void slub_test(void) {
    ...
    for (int s = 0; s < num_sizes; s++) {
        ...
        for (int i = 0; i < TEST_OBJ_NUM / num_sizes; i++) {
            ...
            assert(objs[s][i] != NULL);  // 确保分配成功
            memset(objs[s][i], 0xA0 + (i & 0xF), sz);
        }
    }
    cprintf("【阶段1】分配完成。\n\n");

    /* 阶段 2：释放部分对象，测试 slab 回到部分使用 */
    cprintf("【阶段2】释放部分对象...\n");
    for (int s = 0; s < num_sizes; s++) {
        size_t sz = sizes[s];
