...
=> 0x80000000:	csrr	a6, mhartid         # 从CSR寄存器mhartid中读取当前硬件线程ID，保存到a6
   0x80000004:	bgtz	a6, 0x80000108      # 如果a6 > 0，跳转到0x80000108
   0x80000008:	auipc	t0, 0x0             # t0 = PC高位（0x80000008） + 0
   0x8000000c:	addi	t0, t0, 1032        # t0 = t0 + 1032
   0x80000010:	auipc	t1, 0x0             # t1 = PC高位（0x80000010） + 0
   0x80000014:	addi	t1, t1, -16         # t1 = t1 - 16
   0x80000018:	sd	    t1, 0(t0)           # *(t0) = t1
   0x8000001c:	auipc	t0, 0x0             # t0 = PC高位（0x8000001c） + 0
   0x80000020:	addi	t0, t0, 1020        # t0 = t0 + 1020
   0x80000024:	ld	    t0, 0(t0)           # t0 = *(t0)，从内存中取回之前保存的t1值
```
...
2. 它们主要完成了哪些功能？
   ...
   主要通过一系列的操作，初始化a0,a1,a2寄存器，并将t0寄存器的值赋成OpenSBI.bin的起始地址，并进行跳转，后续OpenSBI初始化处理器的运行环境（具体单行指令的作用见调试过程小节）。
